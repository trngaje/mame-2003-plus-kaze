...
#ifdef CMD_PLUS
/* 반투명박스 */
extern UINT16 use_ex_pal;
extern UINT32 ex_pal_idx;
extern UINT32 org_totalcolors;
extern rgb_t  ex_pal_shcolor;	// 믹싱될 기본 색상(블랙)
extern UINT32 ex_pal_opaque;	// 기본 색상의 불투명도
void palette_update_extpalette(rgb_t color);
void fillbitmap_ex(struct mame_bitmap *dest,pen_t pen,const struct rectangle *clip)
{
	int sx,sy,ex,ey,y,x;
	...
	if (dest->depth == 32)  // RGB triplets (888)
	{
		UINT32 *sp, px[4], ppx[4];
		UINT16 rev_opaque;

		rev_opaque = 100 - ex_pal_opaque;

		px[0] = ((pen >> 24) & 0xff) * ex_pal_opaque;
		px[1] = ((pen >> 16) & 0xff) * ex_pal_opaque;
		px[2] = ((pen >>  8) & 0xff) * ex_pal_opaque;
		px[3] = ((pen      ) & 0xff) * ex_pal_opaque;

		for (y = sy;y <= ey;y++) {
			sp = (UINT32 *)dest->line[y];
			for (x = sx;x <= ex;x++) {
				ppx[0] = ((((sp[x] >> 24) & 0xff) * rev_opaque) + px[0]) / 100;
				ppx[1] = ((((sp[x] >> 16) & 0xff) * rev_opaque) + px[1]) / 100;
				ppx[2] = ((((sp[x] >>  8) & 0xff) * rev_opaque) + px[2]) / 100;
				ppx[3] = ((((sp[x]      ) & 0xff) * rev_opaque) + px[3]) / 100;
				sp[x] = (ppx[0] << 24) | (ppx[1] << 16) | (ppx[2] << 8) | (ppx[3] & 0xff);
			}
		}
	}
	else if (dest->depth == 15 )	// RGB triplets (555)
	{
		UINT16 *sp, px[3], ppx[3], rev_opaque;

		rev_opaque = 100 - ex_pal_opaque;

		px[0] = ((RGB_RED(pen)   >> 3) & 0x1f) * ex_pal_opaque;
		px[1] = ((RGB_GREEN(pen) >> 3) & 0x1f) * ex_pal_opaque;
		px[2] = ((RGB_BLUE(pen)  >> 3) & 0x1f) * ex_pal_opaque;

		for (y = sy;y <= ey;y++) {
			sp = (UINT16 *)dest->line[y];
			for (x = sx;x <= ex;x++) {
				ppx[0] = ((((sp[x] >> 10) & 0x1f) * rev_opaque) + px[0]) / 100;
				ppx[1] = ((((sp[x] >>  5) & 0x1f) * rev_opaque) + px[1]) / 100;
				ppx[2] = ((((sp[x]      ) & 0x1f) * rev_opaque) + px[2]) / 100;
				sp[x] = ((ppx[0] & 0x1f) << 10) | ((ppx[1] & 0x1f) << 5) | (ppx[2] & 0x1f); 
			}
		}
	}
	else if ( dest->depth == 16) 
	{
		// 65536 Full Index Mode - RGB triplets (555) 모드로 해도 무방해 보임.(^^)
		if( Machine->drv->total_colors == 65536 ) {
			UINT16 *sp, px[3], ppx[3], rev_opaque;
			...
			if( use_ex_pal ) {
				// Palette Mode - 반투명용 확장팔레트가 있는 경우
				UINT16 *sp;
				UINT32 newpal;

				palette_update_extpalette( pen ); // 현재 지정된 Pen색상과 게임팔레트 믹싱

				for (y = sy;y <= ey;y++) {
					sp = (UINT16 *)dest->line[y];
					for (x = sx;x <= ex;x++) {
						newpal = sp[x];
						if( newpal < org_totalcolors ) newpal += ex_pal_idx;
						if( newpal > 65535 ) newpal = 65535;
						sp[x] = (UINT16)newpal;
					}
				}
			}
			else {
				// Palette Mode - 이제 사용하는 게임이 없을 듯...
				UINT16 *sp, col;

				for (y = sy;y <= ey;y++) {
					sp = (UINT16 *)dest->line[y];
					col = y % 2;
					for (x = sx+col;x <= ex;x+=2) {
						sp[x] = pen;
					}
				}
			}
	...
	{
		// Palette Mode - 전혀 사용하지 않는 듯한 루틴임.
		// 어디에도 Depth를 8로 세팅하는 경우를 찾을 수 없음.
		UINT8 *sp, col;
		for (y = sy;y <= ey;y++) {
			sp = (UINT8 *)dest->line[y];
			col = y % 2;
			for (x = sx+col;x <= ex;x+=2) {
				sp[x] = (UINT8)pen;
			}
