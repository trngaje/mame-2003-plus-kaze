...
#include "ui_pal.h"
UINT8 colorlist[MAX_COLORTABLE][3] =
{
	...
	{ 0x00, 0x99, 0xcc },	// BUTTON_COLOR_AQUA

	{ 0xff, 0x00, 0xff },	// BUTTON_COLOR_SILVER
	{ 0xff, 0xa0, 0x00 },	// BUTTON_COLOR_NAVY
	{ 0xbe, 0xbe, 0xbe },	// BUTTON_COLOR_LIME
	...
	{ 0x59, 0x64, 0xa5 },	//{ 0x64, 0xa0, 0xe8 },	// OSDBAR_COLOR_FRAMEMEDIUM
	{ 0x88, 0x90, 0xcc },	//{ 0xa0, 0xc8, 0xf0 },	// OSDBAR_COLOR_FRAMELIGHT
	{ 0x33, 0x44, 0x77 },	//{ 0x90, 0xc0, 0xf0 },	// OSDBAR_COLOR_FRAMEDARK
	{ 0x3c, 0x78, 0xf0 },	// OSDBAR_COLOR_DEFAULTBAR
	{ 0xee, 0xbb, 0x00 },	// CURSOR_COLOR
  { 204,	0,		32	 }, // cmd title color (red)
  { 238,	220,	0		 }, // cmd title color (yellow)
  { 0,	  120,	30	 }, // cmd title color (green)
  { 0,	  60,		180  }, // cmd title color (blue)
  { 115,	0,		170  }, // cmd title color (purple)
  { 224,  224,	224  }, // cmd title color (white)
  { 24,   24,		24   }, // cmd title color (black)
  { 204,  0,		32   }, // cmd title color (white 1)
  { 238,  220,	0		 }, // cmd title color (white 2)
  { 0,    120,	30	 }, // cmd title color (white 3)
  { 0,		60,		180	 }, // cmd title color (white 4)
  { 115,	0,		170	 }, // cmd title color (white 5)
  { 204,	0,		32	 }, // cmd title color (black 1)
  { 238,	220,	0		 }, // cmd title color (black 2)
  { 0,		120,	30	 }, // cmd title color (black 3)
  { 0,		60,		180	 }, // cmd title color (black 4)
  { 115,	0,		170	 }, // cmd title color (black 5)
#ifdef UI_TITLE_MENU
	{ 0x00, 0x00, 0x00 },	// FONT_COLOR_LIGHT
	{ 0xc0, 0xc0, 0xc0 },	// FONT_COLOR_DARK
	{ 0x6e, 0x79, 0xc3 },	// TITLEBAR_START
	{ 0x49, 0x53, 0x8d },	// TITLEBAR_END
#endif /* UI_TITLE_MENU */
...
// 반투명을 위한 확장팔레트 관리를 위한 변수
#ifdef CMD_PLUS
UINT16 use_ex_pal = 0;
UINT32 ex_pal_idx = 0;
UINT32 ex_pal_size = 0;
UINT32 total_expal_size = 0;
UINT32 org_totalcolors = 0;
rgb_t  ex_pal_shcolor = MAKE_RGB(0,0,0);	// 믹싱될 기본 색상(블랙)
UINT32 ex_pal_opaque = 40;					// 기본 색상의 불투명도
#endif /* CMD_PLUS */
...
static int palette_alloc(void)
{
...
#ifdef UI_COLOR_DISPLAY
	int max_total_colors = total_colors;

	if( max_total_colors <= (65536 - MAX_COLORTABLE) ) {
		max_total_colors += MAX_COLORTABLE;
	}
	...
	ex_pal_size = 0;
	total_expal_size = 0;
	ex_pal_opaque = 100 - 0;	// 불투명도 (100 - 투명도)
#endif
	if ( !(Machine->drv->video_attributes & VIDEO_RGB_DIRECT) ) {
		if( max_total_colors < 65534 ) {
#ifdef CMD_PLUS
			use_ex_pal = 1;
			ex_pal_shcolor  = MAKE_RGB(0,0,0);
			org_totalcolors = total_colors;
			ex_pal_idx      = max_total_colors;
#endif

			max_total_colors += total_colors;
			if( max_total_colors > 65536 ) 
				max_total_colors = 65536;

#ifdef CMD_PLUS
			// 기본 게임 팔레트 + 메뉴색상(MAX_COLORTABLE)에서
			// 추가적으로 생성될 확장팔레트의 개수
			ex_pal_size = max_total_colors - ex_pal_idx;

			// 확장팔레트의 수를 감안한 전체 팔레트의 수
			total_expal_size = max_total_colors;
#endif
		}
	}
...
int palette_init(void)
{
...
#ifdef UI_COLOR_DISPLAY
	{
		int	 iStep, iSR, iSG, iSB, iER, iEG, iEB;
		
		iStep = GRADATION_LEVEL;
		...
		for(i=0; i<iStep; i++) {
			colorlist[GRADATION_COLOR+i][0] = iSR + ((iER - iSR) * i) / iStep;
			colorlist[GRADATION_COLOR+i][1] = iSG + ((iEG - iSG) * i) / iStep;
			colorlist[GRADATION_COLOR+i][2] = iSB + ((iEB - iSB) * i) / iStep;			
		}
	}
	...
	switch (colormode)
	{
		...
		case PALETTIZED_16BIT:
		{
			...
			/* map the UI pens */
			if (total_colors_with_ui <= (65536 - MAX_COLORTABLE))
			{
				for (i = 0; i < MAX_COLORTABLE; i++) {
					game_palette[total_colors + i] = adjusted_palette[total_colors + i] = MAKE_RGB(colorlist[i][0],colorlist[i][1],colorlist[i][2]);
					Machine->uifont->colortable[i] = total_colors_with_ui++;
				}
			}
			else
			{
				for (i = 0; i < MAX_COLORTABLE; i++) {
					Machine->uifont->colortable[i] = MAKECOL15(colorlist[i][2],colorlist[i][1],colorlist[i][0]);
				}
			}
		...
		case DIRECT_15BIT:
		{
			...
			/* map the UI pens */
			for (i = 0; i < MAX_COLORTABLE; i++) {
				Machine->uifont->colortable[i] = MAKECOL15(colorlist[i][0],colorlist[i][1],colorlist[i][2]);
			}
			break;
		}
		...
		case DIRECT_32BIT:
		{
			...
			/* map the UI pens */
			for (i = 0; i < MAX_COLORTABLE; i++) {
				Machine->uifont->colortable[i] = MAKECOL32(colorlist[i][0],colorlist[i][1],colorlist[i][2]);
			}
			break;
		}
	}
...
#ifdef UI_COLOR_DISPLAY
	if( result <= (65536-MAX_COLORTABLE) ) result += MAX_COLORTABLE;

	if( use_ex_pal ) result += ex_pal_size;
#else
	if (result <= 65534) result += 2;
#endif /* UI_COLOR_DISPLAY */
...
#ifdef UI_COLOR_DISPLAY
#ifdef CMD_PLUS
void palette_update_extpalette(rgb_t color)
{
	int i, idx, sr, sg, sb;
	UINT16 r, g, b, rev_opaque;
	rgb_t palcolor;

	if ((colormode != PALETTIZED_16BIT) || !use_ex_pal ) return;
	if( ex_pal_shcolor == color ) return;

	ex_pal_shcolor = color;
	rev_opaque = 100 - ex_pal_opaque;

	sr = RGB_RED(ex_pal_shcolor)   * ex_pal_opaque;
	sg = RGB_GREEN(ex_pal_shcolor) * ex_pal_opaque;
	sb = RGB_BLUE(ex_pal_shcolor)  * ex_pal_opaque;

	idx = 0;
	for(i=ex_pal_idx; i<total_expal_size; i++) {
		palcolor = game_palette[idx]; idx++;
		
		r = (RGB_RED(palcolor)   * rev_opaque) + sr;
		g = (RGB_GREEN(palcolor) * rev_opaque) + sg;
		b = (RGB_BLUE(palcolor)  * rev_opaque) + sb;
		r /= 100; g /= 100; b /= 100;

		game_palette[i] = adjusted_palette[i] = MAKE_RGB(r,g,b);
...
#ifdef UI_COLOR_DISPLAY
#ifdef CMD_PLUS
		if( use_ex_pal ) display->game_palette_entries += ex_pal_size;
#endif
#endif /* UI_COLOR_DISPLAY */		
...
static void internal_modify_single_pen(pen_t pen, rgb_t color, int pen_bright)
{
...
#ifdef UI_COLOR_DISPLAY
#ifdef CMD_PLUS
	if( use_ex_pal && ((ex_pal_idx+pen) < 65536) ) {
		UINT16 r, g, b, rev_opaque;

		rev_opaque = 100 - ex_pal_opaque;
		
		r = (RGB_RED(color)   * rev_opaque) + (RGB_RED(ex_pal_shcolor)   * ex_pal_opaque);
		g = (RGB_GREEN(color) * rev_opaque) + (RGB_GREEN(ex_pal_shcolor) * ex_pal_opaque);
		b = (RGB_BLUE(color)  * rev_opaque) + (RGB_BLUE(ex_pal_shcolor)  * ex_pal_opaque);
		r /= 100; g /= 100; b /= 100;
		game_palette[ex_pal_idx+pen] = adjusted_palette[ex_pal_idx+pen] = MAKE_RGB(r,g,b);
	}
#endif
#endif /* UI_COLOR_DISPLAY */
	...
	if (adjusted_color != adjusted_palette[pen])
	{
		...
		if( use_ex_pal && ((ex_pal_idx+pen) < 65536) ) {
			...
			r = (RGB_RED(adjusted_color)   * rev_opaque) + (RGB_RED(ex_pal_shcolor)   * ex_pal_opaque);
			g = (RGB_GREEN(adjusted_color) * rev_opaque) + (RGB_GREEN(ex_pal_shcolor) * ex_pal_opaque);
			b = (RGB_BLUE(adjusted_color)  * rev_opaque) + (RGB_BLUE(ex_pal_shcolor)  * ex_pal_opaque);
			r /= 100; g /= 100; b /= 100;
			adjusted_palette[ex_pal_idx+pen] = MAKE_RGB(r,g,b);
		}
#endif
#endif /* UI_COLOR_DISPLAY */
...
#ifdef UI_COLOR_DISPLAY
#ifdef CMD_PLUS
				if( use_ex_pal && ((ex_pal_idx+pen) < 65536) ) mark_pen_dirty(ex_pal_idx+pen);
#endif
#endif /* UI_COLOR_DISPLAY */				
				break;
